...
EXTERNAL_FOLDER=$(find /storage -type d -name "com.termux" 2>/dev/null)/files
...
move_to_external(){
    cp -r $HOME/bitcoin-core/.bitcoin/ $EXTERNAL_FOLDER   
    mv $EXTERNAL_FOLDER/.bitcoin $EXTERNAL_FOLDER/blockchain
    ...
    sed -i "s+/root/bitcoin-core/.bitcoin+$EXTERNAL_FOLDER/blockchain+g" $HOME/bitcoin-core/bin/start.sh
    sed -i "s+/root/bitcoin-core/.bitcoin+$EXTERNAL_FOLDER/blockchain+g" $HOME/bitcoin-core/bin/stop.sh

    cat > $HOME/bitcoin-core/bin/debug.sh <<EOF
#!/bin/sh
tail -f $EXTERNAL_FOLDER/blockchain/debug.log
EOF
